...
class EfficientFaceTemporal(nn.Module):
    ...
    def __init__(
        self, stages_repeats, stages_out_channels, num_classes=7, im_per_sample=25
    ):
    ...
    def forward_stage1(self, x):
        ...
        assert (
            x.shape[0] % self.im_per_sample == 0
        ), "Batch size is not a multiple of sequence length."
...
def init_feature_extractor(model, path):
    ...
    pre_trained_dict = {
        key.replace("module.", ""): value for key, value in pre_trained_dict.items()
    }
...
def get_model(num_classes, task, seq_length):
    model = EfficientFaceTemporal(
        [4, 8, 4], [29, 116, 232, 464, 1024], num_classes, task, seq_length
    )
    return model
...
class MultiModalCNN(nn.Module):
    def __init__(
        self, num_classes=8, fusion="ia", seq_length=15, pretr_ef="None", num_heads=1
    ):
        super(MultiModalCNN, self).__init__()
        assert fusion in ["ia", "it", "lt"], print(
            "Unsupported fusion method: {}".format(fusion)
        )

        self.audio_model = AudioCNNPool(num_classes=num_classes)
        self.visual_model = EfficientFaceTemporal(
            [4, 8, 4], [29, 116, 232, 464, 1024], num_classes, seq_length
        )
        ...
        if fusion in ["lt", "it"]:
            if fusion == "lt":
                self.av = AttentionBlock(
                    in_dim_k=input_dim_video,
                    in_dim_q=input_dim_audio,
                    out_dim=e_dim,
                    num_heads=num_heads,
                )
                self.va = AttentionBlock(
                    in_dim_k=input_dim_audio,
                    in_dim_q=input_dim_video,
                    out_dim=e_dim,
                    num_heads=num_heads,
                )
            elif fusion == "it":
                ...
                self.av1 = AttentionBlock(
                    in_dim_k=input_dim_video,
                    in_dim_q=input_dim_audio,
                    out_dim=input_dim_audio,
                    num_heads=num_heads,
                )
                self.va1 = AttentionBlock(
                    in_dim_k=input_dim_audio,
                    in_dim_q=input_dim_video,
                    out_dim=input_dim_video,
                    num_heads=num_heads,
                )
